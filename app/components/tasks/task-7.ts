 ...
 * ```ts
 * // Upload a new poster
 * const updated = await editCollectionPoster(currentCol, selectedFile, (col) => updateState(col));
 *
 * // Clear/remove the existing poster
 * const cleared = await editCollectionPoster(currentCol, null, (col) => updateState(col));
 * ```
 */
export async function editCollectionPoster(
  ...
  if (!collection || !collection.id) {
    throw new Error('Invalid collection: A collection with a valid ID is required to update its poster.')
  }
  ...
  if (file != null) {
    ...
    if (!validTypes.includes(file.type.toLowerCase())) {
      throw new Error('Unsupported file format. Please upload PNG, JPG, or WEBP images.')
    }
